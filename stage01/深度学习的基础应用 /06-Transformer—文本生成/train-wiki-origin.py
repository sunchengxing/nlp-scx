...
class Config:
    ...
    MAX_VOCABULARY_SIZE = 20000                    # 词表最大词数（含 4 个特殊 token）
    SEQ_LEN             = 128                      # 每条训练样本的序列长度
    EMBED_DIM           = 128                      # Token Embedding 维度 d_model
    HIDDEN_SIZE         = 256                      # FFN 隐层宽度（实际 FF = 256*2）
    NUM_LAYERS          = 2                        # Transformer Block 堆叠层数
    DROPOUT             = 0.3                      # Dropout 概率，防止过拟合
    BATCH_SIZE          = 64                       # 每批样本数
    EPOCH               = 10                       # 训练总轮数
    LEARNING_RATE       = 0.001                    # Adam 初始学习率
    DEVICE              = "cuda" if torch.cuda.is_available() else "cpu"  # 自动选 GPU/CPU
    SAVE_PATH           = "transformer_lm_best-20000.pt" # 验证 PPL 最优时保存路径
...
if __name__ == "__main__":
    ...
    model = TransformerLM(                             # 构建模型
        vocab_size=len(vocab), d=cfg.EMBED_DIM, nhead=4,       # nhead=4，d=128 可整除
        layers=cfg.NUM_LAYERS, ff=cfg.HIDDEN_SIZE*2,           # FF = 512
